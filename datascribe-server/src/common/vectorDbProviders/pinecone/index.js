...
const { curateSources, toChunks } = require('../../vectorDb');
const { WorkspaceDocument, DocumentVector } = require('../../model');
...
export const Pinecone = {
  ...
  addDocumentToNamespace: async function (
    ...
    try {
      ...
      if (cacheResult.exists) {
        ...
        await DocumentVector.bulkCreate(documentVectors);
        // await DocumentVector.bulkInsert(documentVectors);
        return true;
      }
  ...
  deleteDocumentFromNamespace: async function (namespace, docId) {
    ...
    console.log({ docId });
    const knownDocuments = await DocumentVector.findAll({ where: { docId } });
    console.log({ knownDocuments });
    ...
    await DocumentVector.destroy({ where: { [Op.in]: indexes } });
    return true;
  },
